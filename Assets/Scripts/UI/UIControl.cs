...
public class UIControl : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        inventory.onItemChangedCallback += UpdateGeneralInventory;
        uiSlots = inventoryParent.GetComponentsInChildren<InventoryUISlot>();
    ...
    {
        // init general slots
        InitInventoryUiSlots(inventory, uiSlots);
        PlayerController.Instance.playerInventoryController.SetUISlotsTargetInventory(inventory, uiSlots);
    }
    ...
    void UpdateGeneralInventory()
    {
        for (int i = 0; i < uiSlots.Length; i++)
        {
            if (i < inventory.Container.Length)
            {
                ...
                {
                    uiSlots[i].AddItem(inventory.Container[i]);
                }
            }
        }
    }
        ...
        if (mouseItem.item != null)
        {
            ...
            else if (OnGlobalInventory.slot != null)
            {
                ...
                if (OnGlobalInventory.slot.inventorySlot == null)
                {
                    ...
                    int relAmount = PlayerController.Instance.playerInventoryController.inventory.GetCurrentAmount(relItem);
                    PlayerController.Instance.playerInventoryController.RelocateToGlobal(relItem, relAmount, OnGlobalInventory.slot);

                }
                else
                {
                    // if slot is not empty, swap
            ...
            if (mouseItem.hoverObj)
            {
                ...
                if (slot2 == null)
                {
                    ...
                    InventoryUISlot[] targetSlotList = uiSlots;
    ...
    public void DropItemIntoWorld()
    {
        ...
        int itemCount = PlayerController.Instance.playerInventoryController.inventory.GetCurrentAmount(mouseItem.item);
        ...
        if (lootInfo)
        {
            ...
            PlayerController.Instance.playerInventoryController.inventory.RemoveItem(mouseItem.item, itemCount);
